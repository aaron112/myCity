...
public class BuddyHandler
{
	...
	public static void loadBuddiesFromRoster(Roster roster)
	{
		...
		for (RosterEntry entry : entries)
		{
			...
			if (buddies.containsKey(bareAddr))
			{
				...
				buddies.get(bareAddr).setPresence(
			...
			else
			{
				...
				buddies.put(
				         bareAddr,
				         new BuddyEntry(userName, bareAddr, roster
	...
	public static void updateLocation(String bareAddr, double lat, double lon,
	         String time)
	{
		if (buddies.containsKey(bareAddr))
		{
			Log.d(TAG, "updateLocation for " + bareAddr + ": " + lat + ", " + lon
			         + ", " + time);
	...
	public static ArrayList<BuddyEntry> getBuddiesOnMap(GeoPoint center,
	         int latSpan, int lonSpan)
	{
		...
		for (BuddyEntry buddy : buddies.values())
		{
			...
			if (buddy.getPresence().isAvailable() && buddy.isMyCityUser()
			         && lat >= latMin && lat <= latMax && lon >= lonMin
			         && lon <= lonMax)
			{
				res.add(buddy);
			}
	...
	public static boolean isBuddy(String bareAddr)
	{
		return buddies.containsKey(bareAddr);
	}
